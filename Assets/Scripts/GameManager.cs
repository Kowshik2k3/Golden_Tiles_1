...
public class GameManager : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        {
            new LevelData("Easy", 1.1f, 0.9f),
            new LevelData("Medium", 1.3f, 1.1f),
            new LevelData("Hard", 1.5f, 1.3f)
        };
    ...
    public void StartGame()
    {
        ...
        if (MusicManager.SelectedSong != null)
        {
            musicSource.clip = MusicManager.SelectedSong.fullClip;
        }
    ...
    private IEnumerator StartLevel(int index)
    {
        ...
        // ✅ FIX: ALWAYS start music at the beginning of EACH level
        if (musicSource != null && musicSource.clip != null)
        {
            musicSource.Stop();   // safety
            musicSource.Play();
        }
    ...
    {
        if (ProgressStarManager.Instance == null) return;
        if (MusicManager.SelectedSong == null) return;

        int starsEarned = ProgressStarManager.Instance.GetEarnedStars();
        SongData song = MusicManager.SelectedSong;
        ...
        if (starsEarned > bestStars)
        {
            ...
            PlayerPrefs.Save();
            Debug.Log($"Saved {starsEarned} stars for song {song.songId}");
        }
    }
